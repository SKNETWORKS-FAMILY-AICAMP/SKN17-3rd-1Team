...
- **OpenChat 기반 파인튜닝 LLM**: 스티브잡스 페르소나에 맞는 말투와 문체로 재구성한 응답 생성  
- **GPT 모델 (보조적 역할)**: 응답 품질 보완

![시스템 아키텍처](./시스템아키텍처.png)
...
2. **문서 검색**  
   FAISS 벡터 DB에서 의미 유사도가 높은 마케팅 논문 청크 검색
...
![스크린샷 2025-03-28 172115](./WBS.png)
...
- **전처리 과정**:
  ...
  - 데이터셋 생성: 수집된 텍스트 기반 `모델` 활용 `instruction`과 질문에 대한 GPT 응답 결과인 `output` 
- **최종 확보량**: 총 **300개 질의응답 쌍**

## ✅ 파인튜닝 데이터 2. 기업 마케팅 전략 기획안 PDF
...
**구현 내용 요약**
* DB 연동 용으로 `Langchain`의 `Document` 개체로 변환된 마케팅 논문 데이터
